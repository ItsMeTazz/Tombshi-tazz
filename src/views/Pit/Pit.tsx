...
const Pit: React.FC = () => {
  ...
  const handleBuyBonds = useCallback(
    async (amount: string) => {
      ...
      addTransaction(tx, {
        summary: `Buy ${Number(amount).toFixed(2)} TOSHBOND with ${amount} TOMBSHI`,
      });
    },
  ...
  const handleRedeemBonds = useCallback(
    async (amount: string) => {
      ...
      addTransaction(tx, { summary: `Redeem ${amount} TOSHBOND` });
    },
    [tombFinance, addTransaction],
  );
        ...
        {!!account ? (
          <>
            <Route exact path={path}>
              <PageHeader icon={'🏦'} title="Purchase & Redeem TOMBSHI Bonds" subtitle="Earn premiums upon redemption" />
            </Route>
            <StyledBond>
              <StyledCardWrapper>
                <ExchangeCard
                  action="Purchase"
                  fromToken={tombFinance.TOMBSHI}
                  fromTokenName="TOMBSHI"
                  toToken={tombFinance.TBOND}
                  toTokenName="TOSHBOND"
                  priceDesc={
                    !isBondPurchasable
                      ? 'TOMBSHI is over peg'
              ...
              <StyledStatsWrapper>
                ...
                <ExchangeStat
                  tokenName="TOSHBOND"
                  description="Current Price: (TOMBSHI)^2"
              ...
              <StyledCardWrapper>
                <ExchangeCard
                  ...
                  fromTokenName="TOSHBOND"
                  toToken={tombFinance.TOMBSHI}
                  toTokenName="TOMBSHI"
                  priceDesc={`${getDisplayBalance(bondBalance)} WBONDs Available in wallet`}
                  onExchange={handleRedeemBonds}
                  disabled={!bondStat || bondBalance.eq(0) || !isBondRedeemable}
                  disabledDescription={!isBondRedeemable ? `Enabled when TOMBSHI > ${BOND_REDEEM_PRICE} USDC` : null}
                />
              </StyledCardWrapper>
            </StyledBond>
          </>
