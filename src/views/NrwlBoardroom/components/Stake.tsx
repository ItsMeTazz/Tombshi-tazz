...
const Stake: React.FC = () => {
  ...
  const [approveStatus, approve] = useApprove(tombFinance.TOSHARE, tombFinance.contracts.NrwlBoardroom.address);

  const tokenBalance = useTokenBalance(tombFinance.TOSHARE);
  const stakedBalance = useStakedBalanceOnMasonryNrwl();
  const { from, to } = useUnstakeTimerMasonry();

  const stakedTokenPriceInDollars = useStakedTokenPriceInDollars('TOSHARE', tombFinance.TOSHARE);
        ...
        <CardContent>
          <StyledCardContentInner>
            <StyledCardHeader>
              ...
              <Label color="#777" text={'TOSHARE Staked'} />
            </StyledCardHeader>
            <StyledCardActions>
