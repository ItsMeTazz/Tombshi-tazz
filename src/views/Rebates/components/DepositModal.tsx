...
const DepositModal: React.FC<DepositModalProps> = ({ max, onConfirm, onDismiss, tokenName = '', token }) => {
  ...
  function formatOutAmount() {
    ...
    return `Receiving: ${outAmount.toFixed(4)} TOMBSHI ($${(outAmount * rebateStats.tombPrice * ftmPrice).toFixed(4)})`;
  }
  ...
  return (
    <Modal>
      ...
      <StyledMaxText style={{ color: 'black' }}>
        {rebateStats.tombAvailable > 0 ? `${rebateStats.tombAvailable.toFixed(4)} TOMBSHI Available` : 'Bond Sold Out'}
      </StyledMaxText>
      <ModalActions>
        <Button
