...
const Lottery: React.FC = () => {
  ...
  const handleBuyBonds = useCallback(
    async (amount: string) => {
      ...
      addTransaction(tx, {
        summary: `SEND ${Number(amount).toFixed(2)} TOMBSHI TO THE LOTTERY ${amount} `,
      });
    },
...
<h2 style={{ textAlign: 'center', marginBottom: '8%', marginTop: '2%', margin: '1%', textTransform: 'capitalize', 
        ...
        WIN USDC BY DEPOSITING TOMBSHI TOKENS! <br /><br />
</h2>
<h2 style={{ textAlign: 'center',  marginBottom: '3%',
        ...(isDesktop ? { fontSize: '1.4rem' } : { fontSize: '1.1rem'}) }}>
        TOTAL TOMBSHI tokens entered: {Number(raffleBals)} <br />
        TOTAL Lottery entries: {Number((raffleBals))/(5)} <br /> <br />

       {/* Total Lottery entries: {Number(raffleBals)} <br /> */}
        Your TOMBSHI tokens entered: {Number(userBals)} <br /> 
        Your Lottery entries: {Number((userBals))/(5)} <br /> <br />
        Should you have any issues 
      sending TOMBSHI via the modal below, you can send TOMBSHI directly to the Lottery address, found <a href="https://snowtrace.io/address/0x4b8d4EAe1Bff52113d7DA8a7103e6fF9f2611A86">here!</a>
        </h2>
</BondCard>
</Grid>
...
<h2 style={{ textAlign: 'left', marginBottom: '5%', marginTop: '5%', 
      ...
      <i>How does it work?  <br /></i>
        Deposit TOMBSHI to be eligible to win USDC prizes. The current price of TOMBSHI is ${tombPriceInDollars} 
        &nbsp;and 5 TOMBSHI = 1 Lottery entry. Winners are randomly selected! 
...
<h2 style={{ textAlign: 'left', marginBottom: '5%', marginTop: '5%', 
      ...
      Click the "Enter Lottery" button below and send at least 5 TOMBSHI. 5 TOMBSHI = 1 Lottery entry. The more 
...
<Grid item xs={12} sm={6}>
  ...
  <ExchangeCard
        ...
        fromToken={grapeFinance.TOMBSHI}
        fromTokenName="TOMBSHI"
       ...
       disabled
        ...
        ...(isDesktop ? { fontSize: '1.2rem' } : { fontSize: '1.0rem'}) }}>The Tombshi lottery is now closed. Winners 
...
{account}
    ...
    {/*
        <h2 style={{ textAlign: 'left', marginBottom: '5%', marginTop: '5%', 
        ...(isDesktop ? { fontSize: '1.3rem' } : { fontSize: '1.1rem'}) }}>
<i>The first inaugural Tombshi has concluded and winners notified!   
